...
Запуск:
  ...
  python seed.py --normalize     # НОРМАЛИЗОВАТЬ education_level ('СПО'->'SPO', 'ВО'->'VO') без ORM
...
# ---- НОРМАЛИЗАЦИЯ education_level БЕЗ ORM (чтобы не падать на Enum) ----
def normalize_groups_education():
    """
    Меняет 'СПО' -> 'SPO', 'ВО' -> 'VO' в таблице групп, минуя ORM.
    Работает даже если чтение через ORM падает на Enum.
    """
    updated_total = 0
    # Основное имя таблицы — "group" (зарезерв. слово, поэтому КАВЫЧКИ ОБЯЗАТЕЛЬНЫ)
    # Добавлен fallback на 'groups' — на случай если у тебя другое __tablename__.
    candidates = ['"group"', 'groups']

    for tbl in candidates:
        try:
            res1 = db.session.execute(
                text(f"UPDATE {tbl} SET education_level = :norm WHERE education_level = :bad"),
                {"norm": "SPO", "bad": "СПО"},
            )
            ...
            updated = (res1.rowcount or 0) + (res2.rowcount or 0)
            db.session.commit()
    ...
    print(f"[normalize] updated rows: {updated_total}")

# ---- сиды справочников ----
def seed_base_dicts():
    ids = {}

    # Group (unique by code) — ставим только латиницу SPO/VO
    ...
    if Teacher is not None:
        t1, _ = get_or_create(Teacher, full_name="Иванов И.И.",
                              defaults=dict(last_name="Иванов", first_name="Иван", middle_name="Иванович"))
        t2, _ = get_or_create(Teacher, full_name="Петров П.П.",
    ...
    # Buildings (уникально по code или name)
    ...
    if TimeSlot is not None:
        ...
        def ord_no(ts): return getattr(ts, "order_no", getattr(ts, "order", 0))
...
def seed_lessons_today(ids):
    ...
    def lesson_exists(order_no):
        ...
        conds = []
        if has_attr(Lesson, "date"):      conds.append(Lesson.date == today)
...
def main():
    ...
    parser.add_argument("--normalize", action="store_true", help="normalize group.education_level to SPO/VO (raw SQL)")
